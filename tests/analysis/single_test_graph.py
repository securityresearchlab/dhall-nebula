...
def img(df, good, bad, maxT, title, name):
	...
	plt.title(title)
	ax.set_ylim([0, maxT])
	ax.scatter(good['date'], good['time'], marker='o', s=1, color='blue', label='Successo', rasterized=True)
	ax.scatter(bad['date'], bad['time'], marker='o', s=1, color='pink', label='Fallimento', rasterized=True)
	ax.plot(df['date'], mean, label='Media', linestyle='--', color='red', rasterized=True)
	plt.ylabel('time (s)')
